 ...
 *	eventually.
 */
 //! __alloc_size marks all allocation sites?
static __always_inline __alloc_size(1) void *kmalloc_noprof(size_t size, gfp_t flags)
{
	//! can we infer the size at compile time?
	if (__builtin_constant_p(size) && size) {
		unsigned int index;

		//! if (size > 8192) use this codepath which invokes the page allocator
		//! (max cache is kmalloc-8k)
		//! this if-statement will be elided at compile time
		if (size > KMALLOC_MAX_CACHE_SIZE)
			return __kmalloc_large_noprof(size, flags);

		//! fetch the kmalloc cache at compile time for performance
		//! (both this and __kmalloc_noprof end up in `slab_alloc_node`)
		//! this is why you'll often see `__kmalloc_cache_noprof` in decompilations
